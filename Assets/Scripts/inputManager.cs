...
public class inputManager : MonoBehaviour
{
    ...
    public touchButtonsLog touchControls;
    ...
    private void Awake(){
        ...
        touchControls = GameObject.Find("touchButtons").GetComponent<touchButtonsLog>();
    ...
    {
        vertical   = Input.GetAxis("Vertical"); //they both range from -1 to 1
        horizontal = Input.GetAxis("Horizontal");
        if (vertical   == 0)   vertical = touchControls.verticalTouch;
        if (horizontal == 0) horizontal = touchControls.horizontalTouch;

        handbrake = (Input.GetAxis("Jump") != 0) ? true : false;
        if (handbrake   == false)   handbrake = touchControls.handBrakeTouch;
       
        boosting = Input.GetKey(KeyCode.LeftShift);
        if (boosting   == false)   boosting = touchControls.boostTouch;
        
        gearUp = Input.GetKey(KeyCode.E);
        if (gearUp   == false)   gearUp = touchControls.gearUpTouch;
        
        gearDown = Input.GetKey(KeyCode.Q);
        if (gearDown   == false)   gearDown = touchControls.gearDownTouch;
